...
export class ItadminManageComponent implements OnInit {
    ...
    createAdmin() {
        if (!this.admin.name || !this.admin.surname || !this.admin.username || !this.admin.email || !this.admin.password || !this.admin.role || !this.admin.gender) {
            this.show('Lütfen tüm alanları doldurunuz.');
        } else {
            const registerEndpoint = 'https://ticket-web-be-6ogu.onrender.com/register';
            const adminRequest = {
            ...
            this.http.post(registerEndpoint, adminRequest).subscribe(
                (response: any) => {
                    ...
                    this.show('Yönetici başarıyla oluşturuldu');
                },
                (error) => {
    ...
    createUser() {
        if (!this.user.name || !this.user.surname || !this.user.username || !this.user.email || !this.user.password || !this.user.role || !this.user.gender) {
            this.show('Lütfen tüm alanları doldurunuz.');
        } else {
            const registerEndpoint = 'https://ticket-web-be-6ogu.onrender.com/register';
            const userRequest = {
            ...
            this.http.post(registerEndpoint, userRequest).subscribe(
                (response: any) => {
                    ...
                    this.show('Üyelik başarıyla oluşturuldu');
                },
                (error) => {
